/*
 * Tencent Video MVL cache tombstone experiment - v2
 * Based on 2026-09-04 腾讯1/腾讯2/腾讯3 HAR experiments.
 *
 * EXPERIMENT ONLY.
 *
 * Goal:
 *   Test whether Tencent Video persists ad modules locally and treats an absent
 *   module as "not updated". Instead of deleting the whole ad module like v1,
 *   this version preserves the module shell/cache identity and removes only
 *   nested ad payloads (Any<AdFeedInfo/AdFocusPoster/AdFeedVideoPoster>).
 *
 * Important:
 *   - Do NOT enable v1 and v2 at the same time.
 *   - This still preserves the 16-byte tRPC frame and rewrites protobuf lengths.
 *   - Non-MVL/non-ad responses pass through unchanged.
 */

const VERSION = "tencent-video-mvl-cache-tombstone-v2-2026.09.04";
const MAX_DEPTH = 24;
...
const TYPE_AD_VIDEO = "type.googleapis.com/com.tencent.qqlive.protocol.pb.AdFeedVideoPoster";
...
if (status && Number(status) !== 200) {
  console.log(`TencentVideo-Tombstone: status=${status}, pass through`);
  $done({});
} else {
  try {
    const input = new Uint8Array($response.bodyBytes || []);
    if (input.length <= TRPC_HEADER_SIZE) {
      console.log(`TencentVideo-Tombstone: body too small=${input.length}, pass through`);
      $done({});
    } else if (!isTrpcFrame(input)) {
      console.log(`TencentVideo-Tombstone: not recognized tRPC frame, pass through bytes=${input.length}`);
      $done({});
    } else {
      ...
      if (!containsAscii(payload, "getMVLPage") &&
        ...
        console.log(`TencentVideo-Tombstone: non-MVL/ad response, pass through bytes=${input.length}`);
        $done({});
      } else {
        const stats = {
          hardShells: 0,
        ...
        const result = rewriteMessage(payload, 0, false, false, stats);
        if (!result.valid || !result.modified) {
          console.log(
            `TencentVideo-Tombstone: no change valid=${result.valid}` +
            ` hardShells=${stats.hardShells}` +
            ` nativeShells=${stats.nativeShells}` +
            ` unchangedShells=${stats.unchangedShells}` +
            ` bytes=${input.length}`
          );
          $done({});
        } else {
          ...
          console.log(
            `TencentVideo-Tombstone: preserved hardShells=${stats.hardShells}` +
            ` nativeShells=${stats.nativeShells}` +
            ` tombstoned=${stats.tombstonedShells}` +
            ` unchangedShells=${stats.unchangedShells}` +
            ` removed adFeedAny=${stats.adFeedAny}` +
            ` adFocusAny=${stats.adFocusAny}` +
            ` adVideoAny=${stats.adVideoAny}` +
    ...
    console.log(`TencentVideo-Tombstone: rewrite error: ${error && error.stack ? error.stack : error}`);
    $done({});
  }
}
...
function isTrpcFrame(bytes) {
  ...
  return readUint32BE(bytes, 4) === bytes.length;
}

function rewriteMessage(bytes, depth, allowRemoveSelf, insideAdShell, stats) {
  if (depth > MAX_DEPTH) {
  ...
  // Remove only the ad payload object itself. Never remove an ad module shell.
  if (allowRemoveSelf && isAdAnyType(direct.anyType)) {
    if (direct.anyType === TYPE_AD_FEED) stats.adFeedAny++;
    else if (direct.anyType === TYPE_AD_FOCUS) stats.adFocusAny++;
    else if (direct.anyType === TYPE_AD_VIDEO) stats.adVideoAny++;
    return { valid: true, modified: true, removeSelf: true, bytes: EMPTY_BYTES };
  }

  const isHardShell = direct.hardAdModule;
  const isNativeShell = direct.nativeAdBlock;
  const isShell = isHardShell || isNativeShell;

  if (isHardShell) stats.hardShells++;
  if (isNativeShell) stats.nativeShells++;

  const childInsideAdShell = insideAdShell || isShell;
  let modified = false;
  const chunks = [];
  ...
  if (isShell) {
    if (modified) stats.tombstonedShells++;
    else stats.unchangedShells++;
  }
...
function classifyDirectMessage(bytes, fields) {
  ...
  for (const field of fields) {
    ...
    if (field.no === 1) {
      ...
      else if (asciiEquals(payload, TYPE_AD_VIDEO)) anyType = TYPE_AD_VIDEO;
    }
...
function isAdAnyType(type) {
  return type === TYPE_AD_FEED || type === TYPE_AD_FOCUS || type === TYPE_AD_VIDEO;
}
